...
unregulated and decentralized environments.
...
and they individually prove knowledge of their respective secret.
...
are the interpolation coefficients for $\{1, \dots, t\}$.
By reverse-engineering Shamir reconstruction,
the proxy tunes $y_i^*,\hspace{2pt} t < i \le n,$
so that the collection $y_1, \dots, y_{t-1}, y_t^*, \dots, y_n^*$
...
meaning that the protocol must be
...
Moreover, while the protocol is probably secure,
...
\vspace{5pt}
\end{equation*}
respectively and send it to the verifier holding $r_i$ in private, $i \in Q$.
\vspace{5pt}
...
the involved shareholders generate Schnorr
...
Specifically, for every $u \in \mathbb{G}$
...
(cf. Proposition \ref{orst_uniqueness_public_key}).
...
\textit{not} reducible to $\mathsf{DL}$ hardness alone,
...
The reduction generalizes the rewinding argument
